...
(define-easy-handler (query-sacraments* :uri "/query-sacraments" :default-request-type :post) ()
  (let ((ip (get-parameter "ip" (remote-addr*) nil))
	(time (get-parameter "time" (now) #'parse-timestring))
	(distance (get-parameter "distance" 25))
	(future (get-parameter "future" 5))
	(maxresults (get-parameter "maxresults" 25))
	(sacraments (get-parameter "sacraments" '("Mass" "Confession") #'yason:parse)))
    (let* ((lat-long (latitude-and-longitude ip))
	   (latitude (get-parameter "latitude" (first lat-long)))
	   (longitude (get-parameter "longitude" (second lat-long))))
